...
make_context() {
	...
	export REPO_URL
	REPO_URL=$(cat <<- 'EOF'
		https://github.com/apocomitamatma/apocomitamatma.pl
	EOF
	)
	export COVERAGE_URL
	COVERAGE_URL=$(cat <<- 'EOF'
		https://coverage-badge.samuelcolvin.workers.dev/redirect/apocomitamatma/apocomitamatma.pl
	EOF
	)
	export DOCS_URL
	DOCS_URL=$(cat <<- 'EOF'
		https://apocomitamatma.pl.readthedocs.io/en/latest/
	EOF
	)
	export PYPI_URL
	PYPI_URL=$(cat <<- 'EOF'
		https://pypi.org/project/apocomitamatma.pl/
	EOF
	)
	export TIDELIFT_URL
	TIDELIFT_URL=$(cat <<- 'EOF'
		https://tidelift.com/subscription/pkg/pypi-apocomitamatma.pl?utm_source=pypi-apocomitamatma.pl
	EOF
	)
	export SKELETON
	SKELETON=$(cat <<- 'EOF'
	...
	GH_REPO_ARGS=$(cat <<- 'EOF'
		"apocomitamatma/apocomitamatma.pl" --public --source=./ --remote=upstream --description="Web services for the best math YouTuber in Poland."
	EOF
	)
	export GH_ENSURE_ENV
	GH_ENSURE_ENV=$(cat <<- 'EOF'
		jq -n '{"deployment_branch_policy": {"protected_branches": false,"custom_branch_policies": true}}' | gh api -H "Accept: application/vnd.github+json" -X PUT "/repos/apocomitamatma/apocomitamatma.pl/environments/$1" --silent --input -
	EOF
	)
